...
def load_activity_from_bank(metin_id: str):
    ...
    def normrow(r: dict):
        return {
            str(k).strip().lower(): ("" if r.get(k) is None else str(r.get(k)).strip())
            for k in r.keys()
        }
    ...
    def get_opt(r, letter):
        for c in [letter.lower(), letter.lower().strip(), letter.strip().lower()]:
            v = r.get(c)
...
def _story_replace_synonyms(s: str) -> str:
    ...
    synonym_map = {
        "yuva": ["ev", "evi", "barinak", "barınağı", "barinagi", "barınak"],
        ...
        "arkadas": ["dost", "arkadaşı", "arkadasi", "arkadaş"],
        "yardimlasma": ["dayanisma", "dayanışma", "birlikte yardim", "birlikte yardım"],
        "ev": ["yuva", "barinak", "barınak"],
        "cocuk": ["ogrenci", "öğrenci", "kiz", "kız", "erkek cocuk", "erkek çocuk"],
        "ogretmen": ["öğretmen", "hoca"],
        "anne": ["anneciği", "annesi"],
        "baba": ["babası", "babasi"],
    }
...
"""
    user = json.dumps(
        {"alan": field_name, "ogrenci_cevabi": answer, "metin": (metin or "")[:5000]},
        ensure_ascii=False,
    )
    try:
...
# =========================================================
    ...
    if baslik:
        st.markdown(
            f"<div class='card'><b>Metnin Başlığı</b><br/>{baslik}</div>",
            unsafe_allow_html=True,
        )
...
# =========================================================
    ...
    st.markdown(
        ...
        <div class="question-box">
            ...
            <div class="fun-note">Dikkatli oku 💪</div>
            <div style="font-size:20px;">{q.get("kok", "")}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    try:
        ...
        # 🔥 aynı soruda tekrar ipucu → daha güçlü ipucu
        hint_level = min(3, max(base_level, st.session_state.hint_clicks_by_q[i]))
        ...
        save_reading_process(
            "AI_HINT",
            f"Soru {i+1} | İpucu sayısı: {st.session_state.hint_clicks_by_q[i]} | {hint}",
            paragraf_no=None
        )
    ...
    with col1:
        if st.button("⬅️ Geri") and i > 0:
            st.session_state.q_idx -= 1
            st.rerun()

    with col2:
        if st.button("İleri ➡️") and i < total_q - 1:
            st.session_state.q_idx += 1
            st.rerun()
